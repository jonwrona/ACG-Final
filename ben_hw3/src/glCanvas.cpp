...
glm::vec3 GLCanvas::TraceRay(double i, double j) {
  ...
  for (int k=0; k<args->num_antialias_samples; k++){
    double x_offset = double(GLOBAL_MTRAND()-0.5);
    double y_offset = double(GLOBAL_MTRAND()-0.5);

    // Here's what we do with a single sample per pixel:
    // construct & trace a ray through the center of the pixle
    double x = (i+x_offset-args->width/2.0)/double(max_d)+0.5;
    double y = (j+y_offset-args->height/2.0)/double(max_d)+0.5;
    Ray r = camera->generateRay(x,y); 
    Hit hit;
    double inverse_sample_size=1.0/double(args->num_antialias_samples);
    glm::vec3 tmp = (raytracer->TraceRay(r,hit,args->num_bounces));
    tmp*=inverse_sample_size;
    color += tmp;
